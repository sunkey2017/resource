...
CREATE TABLE `sys_fileupload` (
	`id` BIGINT(20) NOT NULL COMMENT '主键ID',
	`original_filename` VARCHAR(100) NULL DEFAULT NULL COMMENT '文件原名称',
	`new_filename` VARCHAR(100) NULL DEFAULT NULL COMMENT '文件新名称',
	...
	`is_deleted` INT(11) NOT NULL COMMENT '数据状态:1:删除 0:未删除',
	`remarks` VARCHAR(120) NULL DEFAULT NULL COMMENT '备注',
	PRIMARY KEY (`id`) USING BTREE,
	UNIQUE INDEX `sys_fileupload_id_uindex` (`id`) USING BTREE
)
COMMENT='文件上传记录'
...
----触发器version
------------------------------
create trigger update_version_sys_fileupload
after update
on sys_fileupload for each row
